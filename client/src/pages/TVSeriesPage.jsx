...
function TVSeriesPage() {
  ...
  // called the hook usePopularTVShows
  const setPopularTVShows = async () => {
    setLoading(true);
    await usePopularTVShows();
    setLoading(false);
  };
  setPopularTVShows();

  // get the data from redux store
  const { popularTVShows } = useSelector((state) => state.movie);
  ...
  return (
    <div className="md:px-5">
      ...
      {loading ? (
        <p className="py-40 text-center font-bold text-2xl  md:text-3xl text-[#fc4747]">
          Loading....
        </p>
      ) : (
